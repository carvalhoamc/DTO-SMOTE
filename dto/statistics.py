...
from folders import output_dir, dir_pca_biclasse, metricas_biclasse
...
class Statistics:
		...
		for method in delaunay_preproc_type:
			for mbiclasse in metricas_biclasse:
				...
				df = df[['RANK_ORIGINAL', 'RANK_SMOTE', 'RANK_SMOTE_SVM', 'RANK_BORDERLINE1',
						 'RANK_BORDERLINE2', 'RANK_DELAUNAY']]
	...
	def calcula_media_folds_biclasse(self, df):
		...
		dfr.to_csv(output_dir + 'resultado_media_multiclasse_' + mode + '.csv', index=False)
	...
	def find_best_rank(self, results_dir, tipo):
		...
		self.find_best_delaunay(results_dir, tipo)
	...
	def rank_by_algorithm(self, df, tipo, wd, reducao, order, alpha):
		...
		for name, group in df_temp:
			...
			pre = df_pre[['ORIGINAL', 'SMOTE', 'SMOTE_SVM', 'BORDERLINE1', 'BORDERLINE2', 'GEOMETRIC_SMOTE','DELAUNAY']]
			rec = df_rec[['ORIGINAL', 'SMOTE', 'SMOTE_SVM', 'BORDERLINE1', 'BORDERLINE2','GEOMETRIC_SMOTE', 'DELAUNAY']]
			spe = df_spe[['ORIGINAL', 'SMOTE', 'SMOTE_SVM', 'BORDERLINE1', 'BORDERLINE2','GEOMETRIC_SMOTE', 'DELAUNAY']]
			f1 = df_f1[['ORIGINAL', 'SMOTE', 'SMOTE_SVM', 'BORDERLINE1', 'BORDERLINE2', 'GEOMETRIC_SMOTE','DELAUNAY']]
			geo = df_geo[['ORIGINAL', 'SMOTE', 'SMOTE_SVM', 'BORDERLINE1', 'BORDERLINE2','GEOMETRIC_SMOTE', 'DELAUNAY']]
			iba = df_iba[['ORIGINAL', 'SMOTE', 'SMOTE_SVM', 'BORDERLINE1', 'BORDERLINE2','GEOMETRIC_SMOTE', 'DELAUNAY']]
			auc = df_auc[['ORIGINAL', 'SMOTE', 'SMOTE_SVM', 'BORDERLINE1', 'BORDERLINE2', 'GEOMETRIC_SMOTE','DELAUNAY']]
			...
			df_pre.to_csv(
				wd + reducao + '_total_rank_' + tipo + '_' + order + '_' + str(alpha) + '_' + name + '_pre.csv',
				index=False)
			df_rec.to_csv(
				wd + reducao + '_total_rank_' + tipo + '_' + order + '_' + str(alpha) + '_' + name + '_rec.csv',
				index=False)
			df_spe.to_csv(
				wd + reducao + '_total_rank_' + tipo + '_' + order + '_' + str(alpha) + '_' + name + '_spe.csv',
				index=False)
			df_f1.to_csv(wd + reducao + '_total_rank_' + tipo + '_' + order + '_' + str(alpha) + '_' + name + '_f1.csv',
						 index=False)
			df_geo.to_csv(
				wd + reducao + '_total_rank_' + tipo + '_' + order + '_' + str(alpha) + '_' + name + '_geo.csv',
				index=False)
			df_iba.to_csv(
				wd + reducao + '_total_rank_' + tipo + '_' + order + '_' + str(alpha) + '_' + name + '_iba.csv',
				index=False)
			df_auc.to_csv(
				wd + reducao + '_total_rank_' + tipo + '_' + order + '_' + str(alpha) + '_' + name + '_auc.csv',
				index=False)
			...
			# grafico CD
			identificadores = ['ORIGINAL', 'SMOTE', 'SMOTE_SVM', 'BORDERLINE1', 'BORDERLINE2', 'GEOMETRIC_SMOTE',delaunay_type]
	...
	def rank_total_by_algorithm(self, tipo, wd, reducao, order, alpha):
		...
		for name in classifiers_list:
			...
			path_name = wd + reducao + '_rank_by_algorithm_' + tipo + '_' + order + '_' + str(alpha) + '_' + name + '_pre.csv'
